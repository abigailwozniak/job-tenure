...
*use "G:/mcr/scratch-m1cls01/data/cps/IPUMS/ten-dw/cps_00099.dta" if year>=1996, clear
keep if year>=1996
keep if age>=40 & age<=64
keep if sex==1
drop if jtyears>=90 
...
save "`outputpath'/fig4.dta", replace

use "`outputpath'/fig4.dta", clear
...
gen c1=-.035 if (year==1990 & q==3) | (year==1990 & q==4) | (year==1991 & q<=1) 
gen c2=-.035 if (year==2001) 
gen c3=-.035 if (year==2007 & q==4) | (year==2008) | (year==2009 & q<=2) 

sort time

# delimit ;
twoway 
    (area c1 time, bcolor(gs13) base(-12)) 
    (area c2 time, bcolor(gs13) base(-12)) 
    (area c3 time, bcolor(gs13) base(-12))
	...
	ylabel(-12(2)0, labels labsize(medium) nogrid angle(horizontal)) 
	ymlabel(-12(1)0, nolabels ticks)
	yline(0,lcolor(black) lwidth(medthick))
	yscale(range(-12 0))
	xtitle("", size(small)) 
	xlabel(1985(5)2020, labels labsize(medium) ticks) 
	xmtick(1983(1)2020, nolabels) 
	legend(order(4 "No controls" 5 "Age" 6 "Age, educ., race, mar st." 7 "Demog., and manuf." 8 "Demog., and full ind. and occ. controls") rows(3) holes(2) size(medium) region(style(none)) margin(zero))
	title("", size(small))
	...
	xsize(6.2)
	name("fig4", replace)
	saving("`outputpath'/fig4.gph", replace)
	;

graph export "`outputpath'/fig4.eps", as(eps) replace;
