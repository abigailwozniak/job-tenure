...
log using `logpath'/appendfig1.log, replace
...
save `outputpath'/appendfig1.dta, replace

use `outputpath'/appendfig1.dta, replace
sort ed year
...
sort edg time 
gen c1=2.9975 if (year==1990 & q==3) | (year==1990 & q==4) | (year==1991 & q<=1) 
gen c2=2.9975 if (year==2001) 
gen c3=2.9975 if (year==2007 & q==4) | (year==2008) | (year==2009 & q<=2) 
...
# delimit ;
twoway 
    (area c1 time, bcolor(gs13) base(-15)) 
    (area c2 time, bcolor(gs13) base(-15)) 
    (area c3 time, bcolor(gs13) base(-15))
	...
	ylabel(-15(3)3, labels labsize(medium) nogrid angle(horizontal)) 
	ymtick(-15(1.5)3)
	yscale(range(-12 3))
	xtitle("", size(small)) 
	...
	ysize(3.75)
	xsize(6.2)
	name("appendfig1", replace)
	saving("`outputpath'/appendfig1.gph", replace)
	;

graph export "`outputpath'/appendfig1.eps", as(eps) replace;
