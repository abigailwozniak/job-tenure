...
gen ageg=1 if age>=22 & age<=29
replace ageg=2 if age>=30 & age<=39
replace ageg=3 if age>=40 & age<=44
replace ageg=4 if age>=45 & age<=49
replace ageg=5 if age>=50 & age<=54
replace ageg=6 if age>=55 & age<=59
replace ageg=7 if age>=60
...
gen male_ind`i'=male*ind_`i'
}

foreach o of numlist 1/14 {
gen male_occ`o'=male*occ_`o'
}
...
local i=1
reg tenure year1983-year2020 [aw=w], nocons
local i=1
foreach yr in 1983 1987 1991 1996 1998 2000 2002 2004 2006 2008 2010 2012 2014 2016 2018 2020 {
...
local i=1
xi: reg tenure year1983-year2020 i.ageg*male [aw=w], nocons
foreach yr in 1983 1987 1991 1996 1998 2000 2002 2004 2006 2008 2010 2012 2014 2016 2018 2020 {
...
local i=1
xi: reg tenure year1983-year2020 i.ageg*male i.hsg*male i.sc*male i.cg*male i.white*male i.black*male i.married*male [aw=w], nocons
foreach yr in 1983 1987 1991 1996 1998 2000 2002 2004 2006 2008 2010 2012 2014 2016 2018 2020 {
...
local i=1
xi: reg tenure year1983-year2020 i.ageg*male i.hsg*male i.sc*male i.cg*male i.white*male i.black*male i.married*male i.manuf*male [aw=w], nocons
foreach yr in 1983 1987 1991 1996 1998 2000 2002 2004 2006 2008 2010 2012 2014 2016 2018 2020 {
...
local i=1
xi: reg tenure year1983-year2020 i.ageg*male i.hsg*male i.sc*male i.cg*male i.white*male i.black*male i.married*male male_ind* male_occ* [aw=w], nocons
foreach yr in 1983 1987 1991 1996 1998 2000 2002 2004 2006 2008 2010 2012 2014 2016 2018 2020 {
...
save "`outputpath'/fig3.dta", replace

***PLOTTING FIGURE

use "`outputpath'/fig3.dta", clear

sort year

foreach var of varlist yearFE-indocc {
sum `var' if year==1983
gen d_`var'=`var'-`r(mean)'
replace d_`var'=0 if year==1983
}
...
gen c1=.5 if (year==1990 & q==3) | (year==1990 & q==4) | (year==1991 & q<=1) 
gen c2=.5 if (year==2001) 
gen c3=.5 if (year==2007 & q==4) | (year==2008) | (year==2009 & q<=2) 
gen line=0 if time==1983
replace line=0 if time==2020
sort time
...
twoway 
	...
	(connected d_manuf time  , msymbol(T) mcolor(black) msize(medium) lcolor(black) lwidth(medium) lpattern(dash))	
	(connected d_indocc time  , msymbol(Th) mcolor(black) msize(medium) lcolor(black) lwidth(medium) lpattern(solid))
	(connected line time  , msymbol(none) mcolor(black) msize(medsmall) lcolor(black) lwidth(thin) lpattern(solid))
	,
	ytitle("", size(medium) margin(vsmall)) 
	ylabel(-1(.5).5, labels labsize(medium) nogrid angle(horizontal)) 
	ymlabel(-1(.25).5, nolabels ticks)
	yscale(range(-1 .5))
	xtitle("", size(small)) 
	xlabel(1985(5)2020, labels labsize(medium) ticks) 
	xmtick(1983(1)2020, nolabels) 
	legend(order(4 "No controls" 5 "Sex x age" 6 "Sex x age, education, marital status, and race" 7 "Sex x demog. and manuf." 8 "Sex x demg., and ind./occ.") holes(2) rows(4) size(medium) region(style(none)) margin(zero))
	title("", size(small))
	...
	xsize(6.2)
	name("fig3", replace)
	saving("`outputpath'/fig3.gph", replace)
	;

graph export "`outputpath'/fig3.eps", as(eps) replace;
# delimit cr	
