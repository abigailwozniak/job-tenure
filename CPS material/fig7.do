...
log using `logpath'/fig7.log, replace
...
save "`outputpath'/fig7.dta", replace

use "`outputpath'/fig7.dta", clear
...
gen c1=6.1 if (year==1990 & q==3) | (year==1990 & q==4) | (year==1991 & q<=1) 
gen c2=6.1 if (year==2001) 
gen c3=6.1 if (year==2007 & q==4) | (year==2008) | (year==2009 & q<=2) 

sort time

# delimit ;
twoway 
    (area c1 time, bcolor(gs13) base(0.05)) 
    (area c2 time, bcolor(gs13) base(0.05)) 
    (area c3 time, bcolor(gs13) base(0.05))
	...
	ylabel(0(2)6, labels labsize(medium) nogrid angle(horizontal)) 
	ymlabel(0(1)6, nolabels ticks)
	...
	xsize(6.2)
	name("fig7", replace)
	saving("`outputpath'/fig7.gph", replace)
	;

graph export "`outputpath'/fig7.eps", as(eps) replace;
